...
import com.jiminger.gstreamer.BinManager.OnError;
...
public class Branch {
   ...
   /**
    * Conditionally apply the consumer to the Branch
    */
   public Branch doIf(final boolean predicate, final Consumer<Branch> doThis) {
      if(predicate)
         doThis.accept(this);
   ...
   public Element getSinkElement() {
      ...
      if(first == null)
         throw new IllegalStateException("There doesn't appear to be any elements on this " + Branch.class.getSimpleName() + ". Can't retrieve sink element.");
      return first;
   }
      ...
      Arrays.stream(downstream).forEach(b -> {
         if(b.alreadyTeminatedSrcEnd())
            throw new IllegalStateException(
                  "You cannot make this branch " + b + " downstream of a tee because it's already terminated at the src end with either another element/pad.");
   ...
   static void linkFullChainOfBranches(final Branch current) {
      ...
      if(current.afterDownstreamEndPad != null)
         ...
         if(current.afterDownstreamEndElementPadName != null) {
            downstreamPad = element.getSinkPads().stream().filter(p -> current.afterDownstreamEndElementPadName.equals(p.getName())).findFirst().orElse(null);
            if(downstreamPad == null)
               throw new IllegalStateException("Couldn't find a pad named " + current.afterDownstreamEndElementPadName + " among " + element.getSinkPads());
