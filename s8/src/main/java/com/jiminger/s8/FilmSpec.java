...
public class FilmSpec
{
   ...
   public static final String [] filmLayoutDesc = 
    ...
    */
   public static int sprocketLayout(int filmType, int filmLayout, boolean reverseImage)
   {
      return (reverseImage ? sprocketLayoutFromFilmLayoutRev[ filmLayout ] : sprocketLayoutFromFilmLayout[ filmLayout ]);
   }

   public static double filmAttribute(int filmType, int attributeIndex)
   {
      return filmModel(filmType)[attributeIndex];
   }

   public static double [] filmModel(int filmType)
   {
      return filmType == superEightFilmType ? s8Spec : mm8Spec;
   }

   public static double inPixels(int model, int index, double resulutiondpi)
   {
      return filmModel(model)[index] * resulutiondpi / mmPerInch;
   }

   public static boolean isVertical(int filmLayout)
   {
   ...
   public static class SprocketHoleOrder implements Comparator<Point>
   {
      ...
      public SprocketHoleOrder(int filmLayout)
      {
                                          ...
                                          List<WeightedPoint> verifiedClusters)
   {
      ...
      WeightedPoint numOneHole = sprockets.remove(0);
      ...
      int halfSprock = 
         ((int)(FilmSpec.inPixels(filmType,FilmSpec.heightIndex,resolutiondpi) + 0.5) + 1)/2;
   ...
   private static boolean gatherCorrectMatches(Point numOneHole, int filmType, int filmLayout,int resolutiondpi, 
                                               int imgLength, int halfSprock, List<Point> sprockets, 
                                               List<Point> verifiedClusters, int directionMult)
   {
      // how many sprocket holes SHOULD I have.
      int pixBetweenSprockets = 
         (int)(FilmSpec.inPixels(filmType,FilmSpec.frameCenterToFrameCenterIndex,resolutiondpi) + 0.5);

      if (directionMult == -1)
         pixBetweenSprockets = -pixBetweenSprockets;

      int curPos = 
         getPointPos(numOneHole,FilmSpec.isVertical(filmLayout)) + pixBetweenSprockets;
      ...
      for (boolean done = (directionMult == 1) ? (curPos >= imgLength) : (curPos < 0); !done;)
      {
         ...
         {
            int dist = 
               getPointPos(sprockets.get(i), FilmSpec.isVertical(filmLayout)) -
               curPos;
         ...
         if (mindist > halfSprock)
         {
            ...
            //  there is a missing sprocket hole here
                                                         // add 10 for slop
            if ((directionMult == 1 ? (curPos < (imgLength - halfSprock - 10)) :
                 (curPos > (halfSprock + 10))))
            {
         ...
         if (nearest >= 0)
         {
            Point nc = (Point)sprockets.remove(nearest);
   ...
   public static int getPointPos(Object fit, boolean vertical)
   {
