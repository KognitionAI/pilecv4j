...
public class RtpBin implements AutoCloseable {
    ...
    public Bin build(final Bin pipe) {
        ...
        sessions.stream().forEach(s -> {
            ...
            if (s.incommingRtp != null)
                Element.linkPads(s.incommingRtp.element.element, s.incommingRtp.pad.getName(), rtpBin, s.padName(false, true));

            if (s.outgoingRtp != null)
                Element.linkPads(rtpBin, s.padName(false, false), s.outgoingRtp.element.element, s.outgoingRtp.pad.getName());

            if (s.sendRtcp != null)
                Element.linkPads(rtpBin, s.padName(true, false), s.sendRtcp.element.element, s.sendRtcp.pad.getName());

            if (s.recvRtcp != null)
                Element.linkPads(s.recvRtcp.element.element, s.recvRtcp.pad.getName(), rtpBin, s.padName(true, true));
        });
    ...
    private static class ElementPad {
        public final ElementWrap<Element> element;
        public final Pad pad;

        public ElementPad(final ElementWrap<Element> element, final boolean sink) {
            this.element = element;
            final List<Pad> pads = sink ? element.element.getSinkPads() : element.element.getSrcPads();
    ...
    private static class SessionConfig {
        ...
        private void addTo(final Bin pipe) {
            if (incommingRtp != null)
                pipe.add(incommingRtp.element.element);

            if (outgoingRtp != null)
                pipe.add(outgoingRtp.element.element);

            if (recvRtcp != null)
                pipe.add(recvRtcp.element.element);

            if (sendRtcp != null)
                pipe.add(sendRtcp.element.element);
        }
